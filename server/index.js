    ...
    app.post('/register', async (req, res) => {
        try {
            ...
            if (!username || !email || !password || !usertype) {
                console.error("❌ Missing fields:", { username, email, password, usertype });
            ...
            if (usertype === 'freelancer') {
                ...
                console.log("✅ Freelancer profile created:", newFreelancer);
            }
    
            res.status(200).json(user);
        } catch (err) {
            console.error("❌ Error in /register:", err); // Log exact error
